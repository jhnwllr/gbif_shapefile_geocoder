...
General purpose **shapefile geocoder** that gives back **gbifids** for a given **shapefile**. That is, it will give you the points that fall within the polygon of the shapefile. 

Currently it is expected that the shapefile includes only **simple polygons** (no multi-polygons, points, or other shapes).  
...
6. database: Where the hdfs table where the occurrences table ( **.occurrence_pipeline_hdfs** ) i.e. "uat" or "prod_h"

> Since there is are a polygon_id and iso2 column in the country_centroid_shapefile the join columns will be "_c4" and "_c5". Would be "_c3" "_c4" if is there was only one polygon id column.
